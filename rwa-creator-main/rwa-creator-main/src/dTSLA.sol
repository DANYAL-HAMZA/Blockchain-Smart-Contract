...
//Functions client enables us to send chainlink request before we can recieve a response on our TSLA status before minting can take place
import { FunctionsClient } from "@chainlink/contracts/src/v0.8/functions/dev/v1_0_0/FunctionsClient.sol";
// ConfirmOwner is a chainlink new version of OnluyOwner
...
contract dTSLA is FunctionsClient, ConfirmedOwner, ERC20, Pausable {
     ...
     */
    constructor(
     //Anytime we make a call to a chainlink function, we must pay a chainlink token or native gas token. This is governed by our subscriptionId
        uint64 subId,
        string memory mintSource,
        string memory redeemSource,
        address functionsRouter,
        //This tells our functions router which chainlink decentralized network to send our call to
        bytes32 donId,
        ...
        returns (bytes32 requestId)
    {
        ...
        //define the request object used t send mint request information to chainlink
        FunctionsRequest.Request memory req;
        //This line tells chainlink we are going to use a javascript source code 
        ...
        // This function comes with the functionsClient. Send the request and store the request ID.
        //  The encode function encodes our request to CBOR encoded bytes( bytes in which chainlink nodes understands data)
    ...
    function sendRedeemRequest(uint256 amountdTsla) external whenNotPaused returns (bytes32 requestId) {
        ...
        //For the redemption process, we need to tell the brokage how much TSLA to sell for USDC amountDtsla
        // must be sent as a parameter to the script we are going to write
        string[] memory args = new string[](2);
        //import strings from openzeppelin and use for uint256
        ...
        //amount usdc to return for redemption
    ...
    //Returns the amount of TSLA value in USD stored in our broker.
    //If there are enough TSLA tokens, tehn min t dTSLA.
    ...
    function _getCollateralRatioAdjustedTotalBalance(uint256 amountOfTokensToMint) internal view returns (uint256) {
        ...
        // Th ecollateralized ratio and the collateralized precision makes sure users are over collateralized
...
//This returns the new expected total value in USD of all the dTSLA tokens combined
    function getCalculatedNewTotalValue(uint256 addedNumberOfTsla) public view returns (uint256) {
       // If there are 10 dtsla tokens + 5 dtsla tokens = 15 dtsla tokens * tsla price(100$) = 1500
       // so there should be at least 1500$ available in our broker
