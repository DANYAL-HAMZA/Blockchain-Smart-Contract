...
public class BlockChainBankStatement {
    private DefiTransactionRepository defiTransactionRepository;
    private static final String FILE = "C:\\Users\\pc\\Document\\BlockchainBankStatement.pdf";
    private DefiUserRepository userRepository;
    private EmailService emailService;

    public List<DefiTransaction> generateBlockchainStatement(String userAddress, String startDate, String endDate) throws FileNotFoundException, DocumentException {
        LocalDate start = LocalDate.parse(startDate, DateTimeFormatter.ISO_DATE);
        LocalDate end = LocalDate.parse(endDate, DateTimeFormatter.ISO_DATE);

        List<DefiTransaction> transactionList = defiTransactionRepository.findAll().stream().filter(transaction ->
                transaction.getUserAddress().equals(userAddress)).filter(transaction ->
        ...
        DefiUser user = userRepository.findByWalletAddress(userAddress);
       ...
       // Log.info("setting document size");
        ...
        PdfPCell address = new PdfPCell(new Phrase("Customer Wallet Address:" + user.getWalletAddress()));
        address.setBorder(0);
        ...
        transactionList.forEach(transaction -> {
            ...
            transactionTable.addCell(new Phrase(transaction.getTransactionHash().toString()));
